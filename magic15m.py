...
bb_length = 19  # Periodo Bandas Bollinger
bb_mult = 3   # Desviación Bandas Bollinger
atr_length = 13  # Periodo ATR
ma_trend_length = 50  # Periodo MA Tendencia
umbral_volatilidad = 0.02  # Umbral ATR (filtro volatilidad)

# Multiplicadores TP/SL ajustados según la estrategia
tp_multiplier = 3.2  # Take Profit multiplicador
...
TELEGRAM_TOKEN = '8014293001:AAH6rlFfddrTk8XP9nwsaeb3aPlqvxtiaas'
...
def registrar_operacion(fecha, tipo, precio_entrada, cantidad, tp, sl, resultado=None, pnl=None):
    archivo = 'registro_operaciones_magic.csv'
...
def ejecutar_bot_trading():
    ...
    enviar_telegram(f"🛑 **Bot {symbol} - Estrategia MAGIC detenido**\n⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    log_consola("Bot de trading detenido")

# ============ INICIO DEL PROGRAMA ============
if __name__ == "__main__":
    print("🤖 Bot de Control MAGICUSDT iniciado")
